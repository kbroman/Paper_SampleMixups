...
if(file.exists(file)) {
  ...
  n.bad.hypo <- sum(apply(hypo.mlratio, 2, function(a) median(a[!is.na(a) & a > -2 & a < 2]) > 0.016))
...
mice <- lapply(tissues, function(a) colnames(get(paste(a, "mlratio", sep="."))))
...
cor.sub <- cor(kidney.mlratio[wh,"Mouse3567"], liver.mlratio[wh,"Mouse3567"], use="complete")
...
\Sexpr{nrow(kidney.mlratio) %/% 1000},\Sexpr{nrow(kidney.mlratio) %% 1000}
