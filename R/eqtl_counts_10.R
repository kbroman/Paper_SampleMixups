...
# this version is for LOD >= 10

library(broman) # for grayplot
source("colors.R")
attach("../Analysis/R/tissue_text.RData")
attach("../Analysis/R/Rcache/neqtl_new_10.RData")
attach("../Analysis/R/Rcache/neqtl_old_10.RData")
neqtl.old.10 <- t(sapply(neqtl.old.10, colSums))
neqtl.new.10 <- t(sapply(neqtl.new.10, colSums))


postscript("../SuppFigs/figS20.eps", height=4.5, width=6.5,
...
grayplot(x, y, xlab="", ylab="No. local-eQTL", type="n",
         ylim=c(0, 2100), yat = seq(0, 2000, by=500), hlines=seq(0, 2000, by=500),
...
grayplot(x, y, xlab="", type="n",
         ...
         ylim=c(0, 5800), hlines=seq(0, 5000, by=1000),
