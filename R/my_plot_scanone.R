# special version of plot.scanone for plot of insulin LOD curves
my.plot.scanone <- 
function(x,x2,x3,chr,lodcolumn=1,incl.markers=TRUE,xlim, ylim,
         ...
         alternate.chrid=FALSE, bandcol=NULL, bgrect="white",
         hlines=seq(0, 8, by=2), ...)
{
    ...
    if(onechr) {
      ...
      if("ylab" %in% names(dots)) {
        if("xlab" %in% names(dots)) {
          plot(0,0,ylim=ylim,xlim=xlim,type="n",ylab="",...)
        }
        else {
          plot(0,0,ylim=ylim,xlim=xlim,type="n",
               xlab="Map position (cM)",ylab="",...)
        }
      }
      else {
        if("xlab" %in% names(dots)) {
      ...
      if("ylab" %in% names(dots)) {
        ...
        if("xlab" %in% names(dots)) {
          ...
          plot(0,0,ylim=ylim,xlim=xlim,type="n",xaxt="n",
               xlab="Chromosome",ylab="", xaxs="i",
               ...)
        }
      }
    }
  }
  title(ylab="LOD score", mgp=c(1.7, 0.5, 0))
  ...
  if(!add && !onechr) {
    if(!alternate.chrid || length(xtick) < 2) {
      for(i in seq(along=xtick))
        axis(side=1, at=xtick[i], labels=xticklabel[i], tick=FALSE, mgp=c(3, 0.2, 0), cex.axis=0.8)
    }
    else {
...
#        axis(side=1, at=xtick[i], labels="")
        axis(side=1, at=xtick[i], labels=xticklabel[i], line=-0.4, tick=FALSE, mgp=c(3, 0.5, 0))
      }
      for(i in even) {
#        axis(side=1, at=xtick[i], labels="")
        axis(side=1, at=xtick[i], labels=xticklabel[i], line=+0.4, tick=FALSE, mgp=c(3, 0.5, 0))
