...
this_dir <- "."
temp_dir <- tempdir()
raw_path <- file.path(this_dir, raw_zipfile)
clean_path <- file.path(this_dir, clean_zipfile)
...
unzipped_files <- utils::unzip(raw_path, exdir=temp_dir)
...
for(tissue in tissues) {
    ...
    assign(obj,
           t(myfread(file.path(temp_dir, "Raw", paste0(tissue, "_mlratio_raw.csv")))))
...
save(list=paste0(tissues, ".mlratio"), file=file.path("Rcache", "mlratios.RData"))

# read very raw genotypes and make some slight changes
suppressWarnings(rawg <- read.cross("csv", file.path(temp_dir, "Raw"), "genotypes_veryraw.csv",
...
suppressWarnings(f2g <- read.cross("csv", file.path(temp_dir, "Raw"), "genotypes_raw.csv",
...
unzipped_files <- utils::unzip(clean_path, exdir=temp_dir)
...
for(tissue in tissues) {
    ...
    obj <- paste0(tissue, ".mlratio")
    assign(obj,
           myfread(file.path(temp_dir, "Clean", paste0(tissue, "_mlratio_clean.csv"))))
    save(list=obj, file=file.path("..", "FinalData", paste0(tissue, "_mlratio_final.RData")))
}

# copy annotation file
annot <- fread(file.path(temp_dir, "Clean", "microarray_annot.csv"), data.table=FALSE)
save(annot, file=file.path("..", "OrigData", "annot.amit_rev.RData"))

# clean genotype data
f2g <- read.cross("csv", file.path(temp_dir, "Clean"), "genotypes_clean.csv",
                  genotypes=c("BB", "BR", "RR"), alleles=c("B", "R"))
f2g$geno <- f2g$geno[c(1:19, "X")]

# genetic and physical maps
gmap <- data.table::fread(file.path(temp_dir, "Clean", "markers_genetic_map.csv"))
pmap <- data.table::fread(file.path(temp_dir, "Clean", "markers_physical_map.csv"))
mnames <- split(pmap$marker, factor(pmap$chr, levels=c(1:19,"X")))
